...
- _flat_: значение самого местоположения.
- _cum_ (cumulative): значение местоположения плюс все его потомки.
...
_опции_ настраивают содержимое отчета. Каждая опция имеет значение,
...
- **-focus= _regex_**: Включать только сэмплы, содержащие запись отчета, соответствующую
  _regex_ (регулярному выражению).
- **-ignore= _regex_**: Не включать сэмплы, содержащие запись отчета, соответствующую
  _regex_.
- **-show_from= _regex_**: Не показывать записи выше первой, соответствующей
  _regex_.
- **-show= _regex_**: Показывать только записи, соответствующие _regex_.
- **-hide= _regex_**: Не показывать записи, соответствующие _regex_.
...
_и_ значением тега, соответствующим `regex2`.
...
графа, контролируемого опцией _nodecount_.
...
- **-list= _regex_**: Генерирует аннотированный листинг исходного кода для функций,
  соответствующих _regex_, со значениями `flat`/`cum` для каждой строки исходного кода.
- **-disasm= _regex_**: Генерирует аннотированный листинг дизассемблированного кода для
  функций, соответствующих _regex_.
- **-weblist= _regex_**: Генерирует комбинированный аннотированный листинг исходного кода/ассемблера
  для функций, соответствующих _regex_, и запускает веб-браузер для его отображения.
...
- **-diff*base= \_profile***: полезно для сравнения двух профилей. Проценты в
...
в базовом профиле _перед_ вычитанием базового профиля из исходного
...
- **-tls_cert= _/path/to/cert_**: Файл, содержащий клиентский TLS-сертификат
  для использования при получении и символизации профилей.
- **-tls_key= _/path/to/key_**: Файл, содержащий приватный ключ TLS для использования
  при получении и символизации профилей.
- **-tls_ca= _/path/to/ca_**: Файл, содержащий сертификат центра сертификации (CA) для
...
Примечание: переведено с помощью AI.
